...
(function(ext) {
    ...
    $.getScript(url1, function(d1, s1, x1) {
        $.getScript(url2, function(d2, s2, x2) {
            ...
            ros.firmataWrite = new ROSLIB.Topic({
                ros: ros.core,
                name: '/COM60/sub',
            ...
            ros.send = function (v) {
                ...
                ros.firmatWrite.publish(pv);
            };

            ros.firmataRead = new ROSLIB.Topic({
                ros: ros.core,
                name: '/COM60/pub',
                messagetype: 'std_msgs/ByteMultiArray',
            });
    ...
    RosSocketDevice.prototype.open = function() {
        ...
        ros.core.connect('ws://10.211.55.29:9090');
    };
    RosSocketDevice.prototype.close = function() {
        ros.firmataRead.unsubscribe();
        connected = false;
    };
    RosSocketDevice.prototype.set_receive_handler = function(handler) {
        ros.firmataRead.subscribe(function (e) {
            handler(e.data);
        });
    };
    RosSocketDevice.prototype.send = function(data) {
        ros.send(data);
    };
